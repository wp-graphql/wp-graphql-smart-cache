...
class Description {
	...
	public function init() {
		...
		add_filter( 'gettext', [ $this, '_translate_excerpt_text_cb' ], 10, 1 );

		add_filter( sprintf( 'manage_%s_posts_columns', Document::TYPE_NAME ), [ $this, '_add_description_column_to_admin_cb' ], 10, 1 );
		add_action( sprintf( 'manage_%s_posts_custom_column', Document::TYPE_NAME ), [ $this, '_fill_excerpt_content_cb' ], 10, 2 );
		add_filter( sprintf( 'manage_edit-%s_sortable_columns', Document::TYPE_NAME ), [ $this, '_make_excerpt_column_sortable_in_admin_cb' ], 10, 1 );
	}
	 ...
	 */
	public function _translate_excerpt_text_cb( $string ) {
		$post = get_post();
	 ...
	 */
	public function _add_description_column_to_admin_cb( $columns ) {
		// Use 'description' as the text the user sees
		$columns['excerpt'] = __( 'Description', 'wp-graphql-persisted-queries' );
		return $columns;
	}

	public function _fill_excerpt_content_cb( $column, $post_id ) {
		if ( 'excerpt' === $column ) {
			echo esc_html( get_the_excerpt( $post_id ) );
		}
	}

	public function _make_excerpt_column_sortable_in_admin_cb( $columns ) {
